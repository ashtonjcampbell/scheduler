...
import { trashPhoto, restorePhoto, deleteForever, retryPhoto } from "./actions";
      ...
      */}
      <div className="relative aspect-[4/5] bg-stone-100 dark:bg-stone-950">
        ...
        {selectable && (
          <button
            ...
            aria-label={
              isSelected
                ? `Deselect ${photo.original_filename}`
                : `Select ${photo.original_filename}`
            }
      ...
      <figcaption className="space-y-1 p-3">
        {/*
          ...
          Dimensions, file size and the source colour profile were printed on
          every card — three lines of numbers that are identical for every
          photo the pipeline has touched, and therefore say nothing. They are
          still recorded, and a photo whose profile had to be GUESSED still
          shouts about it over the image, because that is the one case where
          the answer differs and matters.
        */}
        <p
      ...
      {cropping && (
        <CropEditor photo={photo} onClose={() => setCropping(false)} />
      )}
    </figure>
  );
}
